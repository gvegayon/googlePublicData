...
genMoreInfo <- function(path, encoding='UTF-8', ext='csv', output=NA) {
################################################################################
# DESCRIPTION:
...
################################################################################  
  ...
  x <- data.frame(cbind(x, description=NA, topic=NA))
    ...
    function(x) {
      ...
      if (x['concept.type']!='dimension') {
        ...
        if (!is.na(x['description'])) {
          ...
          newXMLNode('value', parent=tempnode3, description)
        }        
        
        # Here should start the multilanguage loop
        newXMLNode('value', attrs=c('xml:lang'=lang[1]), parent=tempnode2,
...
################################################################################  
  ...
  # TOPICS
  if (!is.null(unlist(varConcepts$topicid))) {
